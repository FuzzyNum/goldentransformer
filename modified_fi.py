...
class ModifiedFaultInjection:
    def __init__(self, model, batch_size, input_shape=None, layer_types=None, **kwargs):
        ...
        _dummyTensor = torch.randint(
            low=0,
            high=30522,  # BERT vocab size, adjust if needed
            size=dummy_shape,
            dtype=torch.long,
            device=device
        )

        outputs = self.original_model(_dummyTensor, output_hidden_states=True)
        ...
        for i, layer in enumerate(model.children()):
            ...
            if list(layer.children()) == []:
                if "all" in layer_types:
                    ...
                    for j in layer_types:
                        if isinstance(layer, j):
                            # neurons
                            handles.append(
    ...
    def _set_value(self, module, input_val, output):
        ...
        layer_dim = self.layers_dim[self.get_current_layer()-1]
        ...
        # Get the corruption list specific to the current layer
        inj_list = list(
            filter(
                lambda x: self.corrupt_layer[x] == self.get_current_layer(),
                range(len(self.corrupt_layer)),
        ...
        # Get the dimension of the current layer
        ...
        # Handling 2D tensors (e.g., [batch_size, sequence_length])
        if layer_dim == 2:
            for i in inj_list:
                self.assert_inj_bounds(index=i)
                logging.info("Changing value to %f", self.corrupt_value[i])

                # Apply corruption to the 2D output tensor
                output[self.corrupt_batch[i]][self.corrupt_dim1[i]] = self.corrupt_value[i]

        # Handling 3D tensors (e.g., [batch_size, sequence_length, embedding_dim])
        elif layer_dim == 3:
            for i in inj_list:
                self.assert_inj_bounds(index=i)
                logging.info("Changing value to %f", self.corrupt_value[i])

                # Apply corruption to the 3D output tensor
                output[self.corrupt_batch[i]][self.corrupt_dim1[i]][self.corrupt_dim2[i]] = self.corrupt_value[i]

        # Handling 4D tensors (if needed, though uncommon for LLM outputs)
        elif layer_dim == 4:
            for i in inj_list:
                self.assert_inj_bounds(index=i)
                logging.info("Changing value to %f", self.corrupt_value[i])

                # Apply corruption to the 4D output tensor
                output[self.corrupt_batch[i]][self.corrupt_dim1[i]][self.corrupt_dim2[i]][self.corrupt_dim3[i]] = self.corrupt_value[i]

        # Update the layer after corruption
        self.updateLayer()
    ...
    def get_fmaps_H(self, layer):
        # Token length (T) if shape is [B, T, D]
        if len(self.output_size[layer]) >= 2:
            return self.output_size[layer][-2]
        return 1  # fallback


    def get_fmaps_W(self, layer):
        # Transformers don't have spatial width, return dummy 1
        return 1
